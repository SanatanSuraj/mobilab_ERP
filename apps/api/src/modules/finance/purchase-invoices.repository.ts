/**
 * Purchase invoices repository (purchase_invoices + purchase_invoice_lines).
 *
 * Mirror of sales-invoices.repository.ts but for vendor bills. Same lifecycle,
 * same invariants — service layer enforces "only DRAFT is mutable".
 * Additional column: match_status + match_notes (Phase 2 advisory; Phase 3
 * three-way match validator will drive these).
 */
...
function rowToInvoice(r: InvoiceRow): PurchaseInvoice {
  return {
    id: r.id,
    orgId: r.org_id,
...
const SELECT_COLS = `id, org_id, invoice_number, vendor_invoice_no, status,
...
function rowToLine(r: LineRow): PurchaseInvoiceLine {
  return {
    id: r.id,
    orgId: r.org_id,
...
const LINE_COLS = `id, org_id, invoice_id, sequence_number,
                   item_id, grn_line_id, description, hsn_sac,
...
export const purchaseInvoicesRepo = {
  // ── Header ─────────────────────────────────────────────────────────────────
  ...
  async list(
    ...
    if (filters.search) {
      where.push(
        `(invoice_number ILIKE $${i} OR vendor_invoice_no ILIKE $${i} OR vendor_name ILIKE $${i} OR notes ILIKE $${i})`,
      );
      params.push(`%${filters.search}%`);
      i++;
    }
    const whereSql = `WHERE ${where.join(" AND ")}`;
    const countSql = `SELECT count(*)::bigint AS total FROM purchase_invoices ${whereSql}`;
    const listSql = `
      SELECT ${SELECT_COLS}
  ...
  async getById(
    ...
    const { rows } = await client.query<InvoiceRow>(
      `SELECT ${SELECT_COLS} FROM purchase_invoices
  ...
  async createHeader(
    ...
    input: Omit<CreatePurchaseInvoice, "lines"> & { invoiceNumber: string },
  ): Promise<PurchaseInvoice> {
    const { rows } = await client.query<InvoiceRow>(
      `INSERT INTO purchase_invoices (
         org_id, invoice_number, vendor_invoice_no, status,
         vendor_id, vendor_name, vendor_gstin, vendor_address,
         purchase_order_id, grn_id,
         invoice_date, due_date, currency,
         notes, terms, place_of_supply,
  ...
  async updateWithVersion(
  ...
  ): Promise<PurchaseInvoice | "version_conflict" | "not_draft" | null> {
    ...
    if (input.vendorInvoiceNo !== undefined) col("vendor_invoice_no", input.vendorInvoiceNo);
    if (input.vendorId !== undefined) col("vendor_id", input.vendorId);
    if (input.vendorName !== undefined) col("vendor_name", input.vendorName);
    if (input.vendorGstin !== undefined) col("vendor_gstin", input.vendorGstin);
    if (input.vendorAddress !== undefined) col("vendor_address", input.vendorAddress);
    if (input.purchaseOrderId !== undefined) col("purchase_order_id", input.purchaseOrderId);
    if (input.grnId !== undefined) col("grn_id", input.grnId);
    if (input.invoiceDate !== undefined) col("invoice_date", input.invoiceDate);
    if (input.dueDate !== undefined) col("due_date", input.dueDate);
    if (input.matchStatus !== undefined) col("match_status", input.matchStatus);
    if (input.matchNotes !== undefined) col("match_notes", input.matchNotes);
    ...
    const verIdx = i;
    const { rows } = await client.query<InvoiceRow>(
      `UPDATE purchase_invoices SET ${sets.join(", ")}
  ...
  async touchHeader(client: PoolClient, id: string): Promise<void> {
    await client.query(
      `UPDATE purchase_invoices SET updated_at = now() WHERE id = $1`,
      [id],
    );
  },
  ...
  async softDelete(client: PoolClient, id: string): Promise<boolean> {
    const { rowCount } = await client.query(
      `UPDATE purchase_invoices SET deleted_at = now()
        WHERE id = $1 AND deleted_at IS NULL`,
      [id],
    );
    return (rowCount ?? 0) > 0;
  ...
  async listLines(
    ...
    const { rows } = await client.query<LineRow>(
      `SELECT ${LINE_COLS} FROM purchase_invoice_lines
  ...
  async getLineById(
    ...
    const { rows } = await client.query<LineRow>(
      `SELECT ${LINE_COLS} FROM purchase_invoice_lines WHERE id = $1`,
      [id],
    );
  ...
  async addLine(
    ...
    const seq =
      ...
      (await purchaseInvoicesRepo.nextLineSeq(client, invoiceId));
    const { rows } = await client.query<LineRow>(
  ...
  async updateLine(
    ...
    if (input.grnLineId !== undefined) col("grn_line_id", input.grnLineId);
    ...
    if (sets.length === 0) return purchaseInvoicesRepo.getLineById(client, lineId);

    params.push(lineId);
    const { rows } = await client.query<LineRow>(
      `UPDATE purchase_invoice_lines SET ${sets.join(", ")}
        WHERE id = $${i}
        RETURNING ${LINE_COLS}`,
      params,
    );
  ...
  async deleteLine(client: PoolClient, lineId: string): Promise<boolean> {
    const { rowCount } = await client.query(
      `DELETE FROM purchase_invoice_lines WHERE id = $1`,
      [lineId],
    );
    return (rowCount ?? 0) > 0;
  },
