...
--
-- Safety boundary:
--   - Tenant API code MUST use the `mobilab_app` role (99-app-role.sql).
--   - Vendor API code MUST use `mobilab_vendor` and nothing else.
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'mobilab_vendor') THEN
    CREATE ROLE mobilab_vendor
      LOGIN
      NOSUPERUSER
...
GRANT USAGE ON SCHEMA public TO mobilab_vendor;
GRANT USAGE ON SCHEMA audit  TO mobilab_vendor;
GRANT USAGE ON SCHEMA vendor TO mobilab_vendor;

-- Keep mobilab_app explicitly out of the vendor schema so a tenant-side
-- SQL injection cannot peek at vendor admin users or the action log.
REVOKE ALL ON SCHEMA vendor FROM mobilab_app;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES    IN SCHEMA public TO mobilab_vendor;
GRANT USAGE, SELECT                  ON ALL SEQUENCES IN SCHEMA public TO mobilab_vendor;
GRANT EXECUTE                        ON ALL FUNCTIONS IN SCHEMA public TO mobilab_vendor;
...
GRANT SELECT ON ALL TABLES IN SCHEMA audit TO mobilab_vendor;

-- ── vendor schema grants ─────────────────────────────────────────────────
-- vendor.admins — full DML: CRUD of vendor admin accounts.
GRANT SELECT, INSERT, UPDATE, DELETE ON vendor.admins TO mobilab_vendor;

-- vendor.action_log — APPEND-ONLY. Mobilab employees cannot tamper with
-- the audit trail, even their own entries. The ops team edits via
-- superuser out-of-band if a row ever needs correction.
GRANT SELECT, INSERT ON vendor.action_log TO mobilab_vendor;
-- (No UPDATE, no DELETE.)
...
GRANT SELECT, INSERT, UPDATE, DELETE ON vendor.refresh_tokens TO mobilab_vendor;

-- Sequences in the vendor schema (for serial keys, though we don't use any
-- right now — future-proofing).
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA vendor TO mobilab_vendor;

-- ── Default privileges for anything created later by mobilab ─────────────
ALTER DEFAULT PRIVILEGES FOR ROLE mobilab IN SCHEMA public
  GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES    TO mobilab_vendor;
ALTER DEFAULT PRIVILEGES FOR ROLE mobilab IN SCHEMA public
  GRANT USAGE, SELECT                  ON SEQUENCES TO mobilab_vendor;
ALTER DEFAULT PRIVILEGES FOR ROLE mobilab IN SCHEMA vendor
  GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES    TO mobilab_vendor;
